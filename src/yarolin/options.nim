...
proc map2*[V1, V2, V3](
  opt1: Option[V1],
  opt2: Option[V2],
  fn: proc(a: V1, b: V2): V3
): Option[V3] {.effectsOf: fn.} =
...
macro orReturn*[T](opt: sink Option[T], value: untyped): untyped =
  ## Works like `try <#try.m,sinkOption[T]>`_ but instead of causing them
  ## function to return an empty option, it returns the value generated by
  ## ``value``.
  let optSym = genSym(ident = "opt")
  quote do:
    let `optSym` = `opt`
...
proc expect*[T](opt: Option[T], msg: string): T {.inline.} =
  ## Raises an exception if the option doesn't have a value
  if not opt.isSome():
